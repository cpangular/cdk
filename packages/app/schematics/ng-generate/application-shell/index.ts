...
export default function (options: Schema): Rule {
  return chain([addAppShellModulesToModule(options), addAppShellHtmlToTemplate(options)]);
}

function addAppShellHtmlToTemplate(options: Schema) {
  return async (host: Tree) => {
    const modulePath = dirname(
      (await findModuleFromOptions(host, {
        name: 'AppModule',
      }))!
    );
    ...
    if (host.exists(appComponentPathHtml)) {
      const fileContent = readFileSync(join(__dirname, 'files/app.component.html.template'), 'utf8');
...
function addAppShellModulesToModule(options: Schema) {
  return async (host: Tree) => {
    ...
    addModuleImportToModule(host, modulePath, 'ApplicationShellModule', '@cpangular/app/application-shell');
    addModuleImportToModule(host, modulePath, 'CpNgDynamicMaterialThemingModule', '@cpangular/material-dynamic-theming/dynamic');
  };
}
