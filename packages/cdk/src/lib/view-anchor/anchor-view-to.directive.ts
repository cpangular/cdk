import {
    ...
    implements OnChanges, OnDestroy, IViewDirective
  {
    ...
    public anchorViewTo: string | symbol | Array<string | symbol> = [];
    ...
    ngOnChanges(changes: SimpleChanges): void {
      ...
      if (anchorViewToChange) {
        this.service.addView(
          this,
          Array.isArray(anchorViewToChange.currentValue)
            ? anchorViewToChange.currentValue
            : [anchorViewToChange.currentValue]
        );
      }
      const inlineFallbackChange = changes["inlineFallback"] ?? changes['anchorViewToInlineFallback'];
