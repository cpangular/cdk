...
export class ResizeDirective implements OnInit, OnDestroy {
  ...
  public set addSizeCssVarsToParent(value: BooleanInput) {
    ...
    this.removeCustomProperties(
      this.elmRef.nativeElement.parentElement,
      this._cssVarName
    );
  ...
  constructor(private readonly elmRef: ElementRef<HTMLElement>) {
    ...
    this._widthChange$ = this._resize$.pipe(
      distinctUntilChanged(
        (a, b) => a.contentRect.width === b.contentRect.width
      )
    );
    this._heightChange$ = this._resize$.pipe(
      distinctUntilChanged(
        (a, b) => a.contentRect.height === b.contentRect.height
      )
    );
  }
